...
var svg = initSvg();
var loadData = function(ticker, start, end, onDone) {
  if (typeof start === 'function') {
    onDone = start;
    start = null;
  }
  d3.json(dataUrl(ticker, start, end), function(error, json) {
    if (error) throw error;
    $('#ticker').val(json.code);
    $('h1').html(json.code);
    $('#endDate').val(json.data[0][0]);
    $('#startDate').val(json.data[json.data.length-1][0]);
    mData = rewriteData(json);
    processData();
    if (onDone) onDone();
  });
}
...
var DB = 'GOOG';
var dataUrl = function(ticker, startDate, endDate) {
  var ret = "https://www.quandl.com/api/v1/datasets/WIKI/" + ticker + ".json?auth_token=1xkdHCMigj6iDyEoGizy";
  if (startDate) {
...
var rewriteData = function(fileData) {
...
$(document).ready(function() {
  loadData('GOOG', render);
  $('#ticker').on('change', function() {
    loadData($('#ticker').val(), render);
  });
  var reloadAll = function() {
    loadData($('#ticker').val(), $('#startDate').val(), $('#endDate').val(), render);
  };
